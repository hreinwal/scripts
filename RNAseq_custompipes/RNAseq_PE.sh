...
CONF=/srv/attract/Scripts/RNAseq_custompipes/config/drerio_150bp2NCBI.conf
...
" 2>&1 | tee -a run.stout
{ time bash $ScriptPATH/STAR_PE.sh 2>&1 | tee -a run.stout ; } 2>> runtime.log
echo "
...
" 2>&1 | tee -a run.stout
{ time bash $ScriptPATH/alignmentQC_PE.sh 2>&1 | tee -a run.stout ; } 2>> runtime.log
echo "
...
### Final cleanup ### ----------------------------------------------------------
cd $PDIR/seqQC/fastp && rm -fv $(find . -name *.fastq.gz) && \
	cd $PDIR/STAR && rm -fv $(find . -name *.out.bam)
cd $PDIR
