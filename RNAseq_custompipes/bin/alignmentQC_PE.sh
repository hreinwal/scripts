...
home=$(pwd)
CPUs=$[$(nproc)-2]
STR=.out.bam #String to match to find corresponding BAM files 
source $home/config
...
mkdir $home/tmp
for d in $(ls -d */); do 
	cd $d && var=$(echo $d | sed 's,/,,')
	cp -f *$STR $home/tmp/$var'.out.bam'
	cd ..;
done
...
# organize output
for f in *.flagstat; do mv -f $f $home/samtools/flagstat/${f/.out.bam/}; done
for f in *.idxstats; do mv -f $f $home/samtools/idxstats/${f/.out.bam/}; done
...
cd $home/qualimap
for d in *; do mv -v $d ${d/.out.bam/}; done
...
conda deactivate # Deactivate the conda multiqc env again
