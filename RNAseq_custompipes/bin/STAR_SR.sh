...
for SID in $(ls *.fastq.gz); do
    ...
    --readFilesIn $SID \
    --outFileNamePrefix $home/STAR/${SID/.fastq.gz/} \
...
# Now organize the output in ./STAR (cleanup)
cd $home/STAR && rm -rf *SJ.out.tab *_STARgenome *_STARpass1
head *Log.out -n 80 > run.info && rm -f *Log.out
mkdir $home/STAR/STAR_log && mv *Log.*.out ./STAR_log/
...
for f in $(ls *$STR); do
    cut -f 1-2 $f | tail -n +5 > $home/GeneCounts/${f/$STR/_GeneCounts.txt}; 
done

# clip file names down to sample ID
cd $home/GeneCounts
for VAR in $(ls *_GeneCounts.txt); do mv -vnT $VAR ${VAR/p*R/R}; done #matches everything from p to R in string and replaces by R
