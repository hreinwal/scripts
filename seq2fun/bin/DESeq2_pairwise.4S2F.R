##############################################
### DESeq2 DGEA script for Seq2Fun Output ###
#############################################

## Author:   Hannes Reinwald
## Contact:  hannes.reinwald@ime.fraunhofer.de
#setwd("~/DATA/DGEA/fipronil_PID-cdi01/seq2fun/")

## README ## -------------------------------------------------------------------
# This script is desgined to run DESeq2 normalization and statistical testing on RNAseq experiments
# previously analyzed with the Seq2Fun tool. This script will automatically adopt to the k numbers
...
# Requiered Input (=OUTPUT from Seq2Fun): 
# - "S2fid_abundance_table_all_samples_submit_2_expressanalyst.txt" --> basically a count matrix with S2F Ids
# - "S2fid_ortholog_annotation_all_samples.txt" --> S2F id annotation file. Contains KEGG (KO) and GO annotation
#    This file is cruical for GESA or ORA
# - coldata.csv --> sample annotation file. Required columns: Condition; Tank; Substance
#    This file ensures to provide as much sample information as possible and allows to correct for
#    potential batch effects.

# Main Analysis Steps are: 
# 1) RLE normalization (DESeq2) across all samples element of CountMatrix
# 2) Calc LFC (with respect to control) and LFcutOff (upper 70% quantile of abs(LFC values))
...
message("\nImporting S2F Outputs (count matrix & ortho. annotation) & coldata file ...")

## coldata import ----------------------------------------------------------------------
import.coldat = function(df=list.files(pattern = "[Cc]oldata[.]"), ref.lv="[Cc]ontrol",
                         cond="Condition", batch="Tank", subs="Substance"){
  message("Loading: \t",df)
  if(grepl(".csv",df)==T) {
    coldata = read.csv2(file = df, row.names = 1, header = T)
    # In case csv file was not differently exported run read.csv
    if (ncol(coldata) <= 1) {
      coldata = read.csv(file = df, row.names = 1, header = T)
    }
  } else {
    # if not ending with csv import with read.delim2
    coldata = read.delim2(file = df, row.names = 1, header = T)
    if (ncol(coldata) <= 1) {
      # if file was differently formated use read.delim
      coldata = read.delim(file = df, row.names = 1, header = T)
    }
  }
  
  # Fix possible minor import problems
  coldata = droplevels(coldata[which(coldata[,cond] != ""), # filtering for non empty rows
                               which(!(grepl("X.",colnames(coldata))) == T)]) # filtering cols
  coldata[,cond]  = gsub(" ","",coldata[,cond]) #removing any spaces in Condition levels
  coldata[,batch] = gsub("-","_",coldata[,batch])
  
  # To make life easier, replace respective colnames with fixed names
  colnames(coldata)[colnames(coldata) == cond]  <- "Condition"
  colnames(coldata)[colnames(coldata) == subs]  <- "Substance"
  colnames(coldata)[colnames(coldata) == batch] <- "Tank"
  
  # set factor levels
  coldata$Tank      <- as.factor(coldata$Tank)
  coldata$Condition <- as.factor(coldata$Condition)
  coldata$Substance <- as.factor(coldata$Substance)
  substance         <- levels(coldata$Substance) #Extract the name(s) of the tested substance(s)
  
  return(coldata)
# END #
}
coldata = import.coldat()

# relevel factors in Condition in right order:
condition = levels(coldata$Condition)
tmp = as.character(coldata$Condition[grep("Control",coldata$Condition)][1]) #extract control factor level
...
## CountMatrix import ------------------------------------------------------------------
# Import count matrix like file from S2F output:
# S2fid_abundance_table_all_samples_submit_2_expressanalyst.txt
import.s2fMtx = function(file = "S2fid_abundance_table_all_samples_submit_2_expressanalyst.txt"){
  x = read.delim(file, header=T)[-1,]
  row.names(x) = x$X.NAME
  return( x[,-1] %>% mutate_all(as.numeric) )
}
...
## annotation colors for downstream plotting -----------------------------------
...
# Removing low abundant counts based on the min CPM relevance threshold suggested by Verheijen et al 2022.
...
# excludes all genes that do not have at least 75% of their replicates expressed at 1 CPM in
# any one of the experimental conditions. This will remove low abundant gene counts prior DESeq2
...
filterCPMs = function(CPM.matrix, id.ls, minCPM=1, perc=.75){
  ...
  # retrieve only GeneIDs for which all values are TRUE in a row
  keep = names(which(apply(cond,1, function(x){ all(x == T) }) == T))
...
## Filter input matrix 
count.matrix.cl = relevance.filter(count.matrix, id.ls, minCPM=1)
...
p  = 0.05 # padj cutoff
Qt = 0.7  # %-Quantile for effect size cut off (i.e. 0.9 => 90% = top 10% abs(LFC))
...
### SKIP THE FOLLOWING LINES FOR NOW !!! 
# We can run these lines once a Annotation package for C.dipterum was built
skip = T
if(skip == F){
...
for(i in names(res.ls)) { lfcDistPlot(res.ls[[i]], title = i, Ylim = c(0,350) ) }
for(i in names(res.ls)) {
...
DE.Cor.Venn = function(df.x, df.y, pcut = .05, typ.x = "DEG in HE", typ.y = "DEG in LE", title = "",
  ...
  # Exit script if df empty!!!
  # stopifnot(nrow(df) != 0) # <- this will break the code
  if(nrow(df) == 0){
    warning("\nThe pairwise lfc comparions of: '",typ.x,"' vs '",typ.y, "'\ndoes not contain any DEGs!",
            " No DEG corrplots could be generated.")
    # build empty ggplot output 
    gg[["VE"]] = ggplot()
    gg[["COR"]]= ggplot()
  } else {
